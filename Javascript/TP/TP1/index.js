...
function main() {
    ...
    let inputNumber = appendChildToElement(divInput, "input", { type: "number", eventHandler: { event: "keyup", fn: () => { submitData(number, Number(inputNumber.value)) } } })
    appendChildToElement(divInput, "button", { text: "Valider", className: ["btn", "btn-dark"], eventHandler: { event: "onclick", fn: () => { submitData(number, Number(inputNumber.value)) } } })
...
function submitData(number, userNumber) {
